...
impl PromptTemplateManager {
    ...
    fn init_default_templates(&mut self) {
        ...
        self.templates.insert(
            ...
            PromptTemplate {
                ...
                template_content: include_str!("../templates/commit_analysis_simple.hbs").to_string(),
                variables: vec![
                    "commit_id".to_string(),
                    "author".to_string(),
                    "timestamp".to_string(),
        ...
        self.templates.insert(
            ...
            PromptTemplate {
                ...
                template_content: include_str!("../templates/commit_analysis_detailed.hbs").to_string(),
                variables: vec![
                    "commit_id".to_string(),
                    "author".to_string(),
                    "timestamp".to_string(),
        ...
        self.templates.insert(
            ...
            PromptTemplate {
                ...
                template_content: include_str!("../templates/daily_summary_enhanced.hbs").to_string(),
                variables: vec![
                    "analyses".to_string(),
                    "start_date".to_string(),
                    "end_date".to_string(),
    ...
    /// 根据模板类型获取模板
    pub fn get_templates_by_type(&self, template_type: &AIAnalysisTemplate) -> Vec<&PromptTemplate> {
        self.templates
            .values()
            .filter(|t| {
                match (&t.template_type, template_type) {
                    (
                        AIAnalysisTemplate::CommitAnalysis { depth: d1, include_code_review: c1 },
                        AIAnalysisTemplate::CommitAnalysis { depth: d2, include_code_review: c2 },
                    ) => d1 == d2 && c1 == c2,
                    (
                        AIAnalysisTemplate::DailySummary { include_tech_analysis: t1, include_risk_assessment: r1 },
                        AIAnalysisTemplate::DailySummary { include_tech_analysis: t2, include_risk_assessment: r2 },
                    ) => t1 == t2 && r1 == r2,
                    _ => false,
                }
            })
    ...
    /// 更新模板
    pub fn update_template(&mut self, template_id: &str, template: PromptTemplate) -> Result<(), String> {
    ...
    /// 渲染模板
    pub fn render_template(&self, template_id: &str, context: &HashMap<String, String>) -> Result<String, String> {
        let template = self.get_template(template_id)
            .ok_or("模板不存在")?;
        
        // 使用handlebars渲染模板
        let mut handlebars = handlebars::Handlebars::new();
        handlebars.register_template_string("template", &template.template_content)
    ...
    pub fn get_commit_analysis_prompt(
        ...
        context.insert("author".to_string(), format!("{} <{}>", analysis.author, analysis.email));
        ...
        // 格式化文件变更信息
        let files_changed = analysis.files_changed.iter()
            .map(|f| format!("{} ({})", f.file_path, self.format_change_type(&f.change_type)))
            .collect::<Vec<_>>()
            .join("\n");
        ...
        // 添加代码语言信息
        let code_languages: Vec<String> = analysis.files_changed.iter()
    ...
    pub fn get_daily_summary_prompt(
        ...
        // 序列化分析结果
        let analyses_json = serde_json::to_string(analyses)
            .map_err(|e| format!("序列化分析结果失败: {}", e))?;
        context.insert("analyses".to_string(), analyses_json);
        
        // 提取贡献者
        let contributors: Vec<String> = analyses.iter()
    ...
    fn analyze_tech_stack(&self, analyses: &[CommitDetailAnalysis]) -> String {
        ...
        for analysis in analyses {
            for file in &analysis.files_changed {
                ...
                } else if path.contains("spring") || path.contains("express") || path.contains("django") {
    ...
    fn analyze_risk_factors(&self, analyses: &[CommitDetailAnalysis]) -> String {
        ...
        for analysis in analyses {
            ...
            if analysis.insertions > 1000 || analysis.deletions > 1000 {
                risks.push(format!("大量代码变更：+{} / -{}", analysis.insertions, analysis.deletions));
            }
