...
use crate::types::git_types::{
    ...
    CommitDetailAnalysis, ImpactLevel
};
...
pub async fn generate_enhanced_daily_report(
    ...
    println!("开始生成增强版日报...");
    println!("配置信息: 仓库数量={}, 用户数量={}, 开始日期={}, 结束日期={}", 
    ...
    for repo_path in &config.repoPaths {
        ...
        println!("仓库 {} 在 {} 至 {} 期间找到 {} 个提交", 
            repo_path, config.startDate, config.endDate, commits.len());
        ...
        println!("仓库 {} 过滤后有 {} 个提交", repo_path, filtered_commits.len());
        
        // 分析每个提交
        for commit in filtered_commits {
            println!("正在分析提交: {} - {}", commit.hash, commit.message.lines().next().unwrap_or(""));
    ...
    println!("总共找到 {} 个提交，成功分析 {} 个", total_commits_found, all_analyses.len());
...
fn generate_enhanced_report_content(
    ...
    let unique_files: std::collections::HashSet<_> = analyses.iter()
    ...
    for analysis in analyses {
        contributor_stats.entry(analysis.email.clone()).or_default().push(analysis);
    }
    ...
    for (email, user_analyses) in contributor_stats {
        if let Some(first) = user_analyses.first() {
            ...
            for analysis in &user_analyses {
                *impact_counts.entry(format!("{:?}", analysis.impact_level)).or_insert(0) += 1;
            }
...
pub async fn get_commit_cache_status(
    ...
    for commit_id in commit_ids {
        status.insert(commit_id.clone(), cache_manager.cache_exists(&repo_path, &commit_id));
    }
...
pub async fn cleanup_cache(
    ...
    let cache_manager = CacheManager::new(&app_dir);
    cache_manager.cleanup_old_cache(days_old)
