...
use crate::types::git_types::{
    AIAnalysisConfig, AnalysisConfig, AnalysisDepth, AnalysisProgress, CommitDetailAnalysis, Report,
};
use chrono::Utc;
use std::sync::Arc;
/// AI分析相关命令
...
pub async fn analyze_commit_with_ai(
    ...
    if force_refresh.unwrap_or(false) {
        let cache_path = analysis_engine
            .cache_manager()
...
pub async fn batch_analyze_commits(
    ...
    // 发送初始进度
    app_handle
        .emit("analysis-progress", &progress)
    ...
    for (i, (repo_path, commit_id)) in commit_ids.iter().enumerate() {
        ...
        if !analysis_engine
            .cache_manager()
            .cache_exists(repo_path, commit_id)
        {
            // 获取提交信息
    ...
    app_handle
        .emit("analysis-progress", &final_progress)
...
pub async fn generate_ai_enhanced_report(
    ...
    for repo_path in &config.repoPaths {
        ...
        for commit in filtered_commits {
            // 检查缓存
            if let Ok(Some(cached_analysis)) = analysis_engine
                .cache_manager()
                .load_commit_analysis(repo_path, &commit.hash)
            {
                ...
                match git_engine.get_commit_diff(repo_path, &commit.hash) {
                    Ok(diff_info) => {
                        match analysis_engine
                            .analyze_commit(repo_path, &commit.hash, &commit, Some(&diff_info))
                            .await
                        {
                            Ok(analysis) => {
    ...
    let content = if use_ai_summary.unwrap_or(true) && !all_analyses.is_empty() {
        ...
        match analysis_engine
            .generate_ai_summary_report_with_template(
                &all_analyses,
                &config.repoPaths,
                &config.startDate,
                &config.endDate,
...
fn generate_basic_report_content(
    ...
    let important_commits: Vec<_> = analyses
        .iter()
        .filter(|a| {
            a.impact_level == crate::types::git_types::ImpactLevel::Critical
...
/// 设置AI分析配置
#[tauri::command]
pub async fn set_ai_analysis_config(config: AIAnalysisConfig) -> Result<bool, String> {
    // TODO: 保存配置到本地
...
pub async fn get_ai_templates(app_handle: tauri::AppHandle) -> Result<Vec<PromptTemplate>, String> {
    ...
    if manager.is_none() {
        let app_dir = app_handle
            ...
            .map_err(|e| format!("Failed to get app data dir: {}", e))?;
        *manager = Some(
            VersionedTemplateManager::new(&app_dir)
    ...
    for template in versioned_templates {
        ...
        // 获取当前内容
        let content = manager
            .get_template_content(&template.id)
...
pub async fn reset_ai_template(
    ...
    // 还原到系统内置版本
    manager
        .revert_to_builtin_version(&templateId)
